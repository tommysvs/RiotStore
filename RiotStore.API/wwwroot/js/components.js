// Riot logo SVG
const RIOT_LOGO = `<svg class="w-18 h-5" viewBox="0 0 580 160" fill="currentColor">
...
function renderNavbar() {
    ...
    navbar.innerHTML = `
        <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <a href="index.html" class="flex items-center gap-2 text-white hover:text-amber-400 transition">
                ${RIOT_LOGO}
            </a>
    
            <button id="mobile-menu-btn" class="md:hidden text-white hover:text-amber-400 transition">
            ...
            <div id="nav-menu" class="hidden md:flex gap-8 items-center">
                ...
                <a href="catalog.html" class="text-white hover:text-amber-400 transition uppercase text-sm font-semibold">Categorías</a>
                <a href="catalog.html" class="text-white hover:text-amber-400 transition uppercase text-sm font-semibold">En oferta</a>
            </div>
...
function renderFooter() {
    const footer = document.getElementById('booter-container');
    footer.innerHTML = `
        <div class="max-w-7xl mx-auto px-4 py-16">
            <div class="mb-12 pb-12 border-b border-gray-700">
                <div>
                    <a href="index.html" class="flex items-center gap-2 text-white hover:text-amber-400 transition mb-4">
                        ${RIOT_LOGO}
                    </a>
                    <p class="text-gray-400 text-sm">
                        Merchandising oficial de League of Legends Classic. Colecciones exclusivas y ediciones limitadas.
                    </p>
                </div>
            </div>

            <div class="text-center text-gray-500 text-xs">
                <p class="mb-2">© 2026 Riot Merch. Este es un sitio simulado con propósitos de demostración educativa.</p>
                <p>No es un sitio oficial de Riot Games. League of Legends es propiedad intelectual de Riot Games, Inc.</p>
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="bg-gray-50 border-b border-gray-200">
            <div class="max-w-7xl mx-auto px-4 py-3 text-sm">
                ${items.map((item, idx) => `
                    ${idx > 0 ? '<span class="text-gray-400 mx-2">/</span>' : ''}
                    ${item.href
            ? `<a href="${item.href}" class="text-red-600 hover:text-red-700">${item.label}</a>`
